...
async function isTenantAdministrator(applicationContext, tenantId, userEmail) {
  const adminCheckResult = await applicationContext.db.query({
    ...
    sql: `
      ...
      LIMIT 1
    `,
    params: [tenantId, userEmail],
  });

  return (adminCheckResult.rows || []).length > 0;
}

async function getActiveAdministratorCount(applicationContext, tenantId) {
  const adminCountResult = await applicationContext.db.query({
    operation: "tenant.adminCount",
    sql: `
...
module.exports = function buildTenantRouter(applicationContext) {
    ...
    asyncHandler(async (request, response) => {
      ...
      if (subdomain) {
        ...
        if ((subdomainCheckResult.rows || []).length) {
          applicationContext.logger.warn("Tenant create subdomain in use", {
            op: "tenant.create",
            actingUserEmail,
            subdomain,
          });
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      if (!["administrator", "users"].includes(role)) {
        applicationContext.logger.warn("Tenant add user invalid role", { op: "tenant.user.add", actingUserEmail, tenantId });
        return response.status(400).json({ ok: false, error: "role must be administrator|users" });
      }
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      if (currentRole === "administrator" && role !== "administrator") {
        const activeAdminCount = await getActiveAdministratorCount(applicationContext, tenantId);
        if (activeAdminCount <= 1) {
          applicationContext.logger.warn("Tenant update would remove last admin", { op: "tenant.user.update", tenantId });
          return response.status(409).json({ ok: false, error: "tenant must have at least 1 administrator" });
        }
      }
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      if (targetRole === "administrator") {
        const activeAdminCount = await getActiveAdministratorCount(applicationContext, tenantId);
        if (activeAdminCount <= 1) {
          applicationContext.logger.warn("Tenant deactivate would remove last admin", { op: "tenant.user.deactivate", tenantId });
          return response.status(409).json({ ok: false, error: "tenant must have at least 1 administrator" });
        }
      }
